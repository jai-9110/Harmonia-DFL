...
func validateGeneralConfig(config *iconfig.IConfig) {
	...
	vecToken := config.GitUserTokens // ------------------- read config.yml token
    for index, token := range vecToken {
		checkIsToken(index, token )
	}
...
func validateEdgeConfig(config *iconfig.IConfig) {
	...
	if config.EdgeModelRepos == nil || len(config.EdgeModelRepos) == 0 { // config.EdgeModelRepo -> config.EdgeModelRepos
		zap.L().Fatal("Need `edgeModelRepos` Information.")
	}
...
func checkIsToken(index int, node *iconfig.TokenStr) {

	if node.Token == "" {
		zap.L().Fatal("gitUserToken undefined")
	}
    zap.L().Debug(fmt.Sprintf("gitUserToken[%d] : %s", index, node.Token))
